...
// Note: If sending requests from the F50 device itself, change the request port to 8080
...
(() => {
    ...
    window.fetch = async (input, init = {}) => {
        ...
        // Silent verification for anyProxy
        ...
        // Extract path only (without query)
        let urlPath = '';
        try {
        ...
        // Not very useful; mainly for obfuscation
...
const getRD = async (cookie) => {
    if (!cookie) throw new Error('Please provide cookie')
...
const sendSms_UFI = async ({ content, number }) => {
    if (!content) throw new Error('Please provide SMS content')
...
const removeSmsById = async (id) => {
    if (!id) throw new Error('Please provide SMS id')
    const cookie = await login()
    const res = await postData(cookie, {
        goformId: 'DELETE_SMS',
        msg_id: id,
...
const readSmsByIds = async (ids) => {
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
        throw new Error('Please provide an array of SMS ids');
    }
    ...
    for (const id of ids) {
        try {
            ...
            console.error(`Error processing SMS ID ${id}:`, err);
...
const deleteAPNProfile = async (index) => {
    if (index == undefined || index == null) throw new Error('Please provide index')
